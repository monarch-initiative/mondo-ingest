...
sync-subclassof: $(foreach n,$(ALL_COMPONENT_IDS), $(REPORTDIR)/$(n).subclass.confirmed.robot.tsv)
	@rm -f $(REPORTDIR)/sync-subClassOf.confirmed.tsv
	@rm -f $(REPORTDIR)/sync-subClassOf.direct-in-mondo-only.tsv
	@rm -f $(TMPDIR)/sync-subClassOf.added.self_parentage_issues.tsv
	$(MAKE) $(REPORTDIR)/sync-subClassOf.confirmed.tsv
	$(MAKE) $(REPORTDIR)/sync-subClassOf.direct-in-mondo-only.tsv
	$(MAKE) $(TMPDIR)/sync-subClassOf.added.self-parentage-issues.tsv
