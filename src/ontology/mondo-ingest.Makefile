...
$(REPORTDIR)/sync-subClassOf.direct-in-mondo-only.tsv: $(foreach n,$(ALL_COMPONENT_IDS), $(REPORTDIR)/$(n).subclass.direct-in-mondo-only.tsv)
...
#$(REPORTDIR)/%.subclass.direct-in-mondo-only.tsv: tmp/mondo-ingest.db tmp/mondo.db tmp/mondo.sssom.tsv
$(REPORTDIR)/%.subclass.direct-in-mondo-only.tsv: tmp/mondo-ingest.db tmp/mondo.db tmp/mondo.sssom.tsv
	$(call sync_subclassof, $*)
#$(REPORTDIR)/%.subclass.added.robot.tsv:
#$(REPORTDIR)/%.subclass.confirmed.robot.tsv:
...
define sync_subclassof
	...
	--outpath-added $(REPORTDIR)/$(1).subclass.added.robot.tsv \
	--outpath-added-obsolete $(REPORTDIR)/$(1).subclass.added-obsolete.robot.tsv \
	--outpath-confirmed $(REPORTDIR)/$(1).subclass.confirmed.robot.tsv \
	--outpath-direct-in-mondo-only $(REPORTDIR)/$(1).subclass.direct-in-mondo-only.tsv \
	--mondo-db-path $(TMPDIR)/mondo.db \
	--mondo-ingest-db-path $(TMPDIR)/mondo-ingest.db \
	--mondo-mappings-path $(TMPDIR)/mondo.sssom.tsv \
	--onto-config-path metadata/$(1).yml
endef
