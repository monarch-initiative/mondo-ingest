...
def uri_to_curie(uri: str, prefix_map: Dict[str, str] = None) -> str:
    ...
    # todo: Use this solution until I get OAK to work for this use case"""
    obo_purl = "http://purl.obolibrary.org/obo/"
    curie = uri
    uri = curie.replace('<', '').replace('>', '')
    if prefix_map:
...
def load_and_format_tsv(path: str, prefix_map: Dict[str, str]) -> pd.DataFrame:
    ...
    # todo: remove this following line when I'm sure it's not needed
...
def get_labels(
    ...
    df2[term_id_field] = df2[term_id_field].apply(uri_to_curie, prefix_map=prefix_map)
    term_label_map = {}
...
def run_excluded_terms_in_mondo_xrefs(
    ...
    # Add labels
    # TODO: get labels (& use term_label_field)
    #  - then update PR and attach table
...
def cli_validate(d: Dict) -> Dict:
    ...
    # todo: Do this for all path args
...
def cli() -> Dict[str, pd.DataFrame]:
    ...
    del kwargs_dict['onto_name']  # todo: if really don't need, remove this line & remove CLI param & from makefile
    output_xrefs, output_classes = {}, {}
    if 'excluded_terms_in_mondo_xrefs' in kwargs_dict['outpath']:
        d = {k: v for k, v in kwargs_dict.items() if k != 'mondo_path'}
        output_xrefs = run_excluded_terms_in_mondo_xrefs(**d)
    if 'excluded_terms_in_mondo' in kwargs_dict['outpath']:
        d = {k: v for k, v in kwargs_dict.items() if k != 'mondo_mappings_path'}
        # todo: remove this noinspection when done
        # noinspection PyArgumentList
        output_classes = run_excluded_terms_in_mondo(**d)
    outputs = {**output_xrefs, **output_classes}
    return outputs
...
def cli_get_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        '-oi', '--outpath-classes', required=False,
        help='Path for output file for list of excluded terms in Mondo.')
    parser.add_argument(
        '-ox', '--outpath-xrefs', required=False,
