...
# Create an exact synonym copy for all labels using the "fixed" label that has brackets
...
INSERT {
  ...
  ?cls oboInOwl:hasExactSynonym ?cleanLabel .

  [
    rdf:type owl:Axiom ;
    owl:annotatedSource ?cls ;
    owl:annotatedProperty oboInOwl:hasExactSynonym ;
    owl:annotatedTarget ?cleanLabel ;
...
WHERE {
  ...
  BIND(REPLACE(STR(?label), " [\\[(].*[)\\]]$", "") AS ?cleanLabel)
